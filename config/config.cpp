...
void config::get_layers_config(string &str)
{
	...
	int tail = 0;
	 ...
	 for(int i=0; i< layers.size(); i++)
	 {
		 ...
		 {
			 layer = new configData(type ,name);
			 cout << endl ;
		 ...
		 }else if(type == string("LRN"))
		 {
			 ...
			 layer = new configLRN(type, name, input, lrnN, lrnAlpha, lrnBeta);
		 ...
		 }else if(type == string("INCEPTION"))
		 {
			 ...
			 layer = new configInception(type, name, input, one, three, five, three_reduce, five_reduce,
		 ...
		 if(type == std::string("DATA")){
			 ...
			 _layerMaps[layer->_input]->_next=layer;
			 _layerMaps[name]->_prev = _layerMaps[layer->_input];
		 	}
